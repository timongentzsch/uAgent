...
const char* UsageText() {
  return "usage: uagent [--yolo] [--json|--json-stream] [--budget USD] "
         ...
         "  -p PROMPT               run one turn, print only the final answer, "
         "exit\n"
         "  --yolo                  approve every requested mutation "
         "automatically\n"
         "  --json                  emit a stable JSON envelope in headless "
         "mode\n"
         "  --json-stream           emit versioned JSONL events in headless "
         "mode\n"
         "  --budget USD            cap total session spend between model "
         "calls\n"
         "  --no-memory             disable memory recall and writes for this "
         "session\n"
         "  --debug[=PATH]           write a sensitive reconstructable JSONL "
         "trace\n"
         "  --attach PATH            send an image or document with the first "
         "message\n"
         "  -c                       resume the most recent saved session\n"
         "  --resume                 pick a saved session to resume at "
         "startup\n"
